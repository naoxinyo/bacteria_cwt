...
import copy
from utils import loss, dataset_no_aug
...
def train_model(args, model):            #训练模型
    ...
    load_dataset = dataset_no_aug.PointsFolderNoAug(data, label)
    ...
    for epoch in range(args.epochs):
        ...
        wandb.log({
            'Loss_train/RCBAM': running_loss_RCMAB / len(train_dataset),
...
def evaluate(args, model, epoch, test_loader, test_dataset):
    ...
    testing_correct_correctNext_V2 = 0
